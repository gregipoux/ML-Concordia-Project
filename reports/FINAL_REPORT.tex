...
The story these two figures tell is simple and robust.
...
tree-based model relies on. We considered this the project's first real
insight beyond ``tune a classifier''.
...
cost on this problem and \textbf{did not deploy them}. This decision is
documented because ensemble-by-default is a common reflex that we wanted
to push back on with data.

\subsection{Champion selection}\label{champion-selection}

We deployed \textbf{Random Forest} as the champion. The justification,
which we rehearsed for the demo:
...
\item
  ...
  than a false positive (extra alert). Recall on Attack is therefore our
  sentinel.
\end{itemize}
...
100-sample background, but the results were too noisy to be actionable
in the time budget, and Tree SHAP was orders of magnitude faster
...
The UI has three parts, all on one page, and is the piece we are most
proud of:
...
\item
  ...
  model files, and we wanted to close that write vector.
\end{itemize}
...
which would not resolve inside a Linux container. MLflow is available
locally via \texttt{mlflow\ ui} from the \texttt{repo/} directory if a
reader of this report wants to inspect the runs.
...
The most useful thing we leave behind is not the champion F1, but the
honest picture of why it sits where it does --- and a UI that surfaces
that picture in five seconds rather than five pages.
